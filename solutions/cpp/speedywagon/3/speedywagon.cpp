...
namespace speedywagon {
    ...
    int activity_counter(const pillar_men_sensor *sensor_array_ptr,int capacity){
        int activity_total{0};
        for (int i = 0; i < capacity; ++i)
        {
            if (connection_check(sensor_array_ptr + i))
            {
                activity_total += (sensor_array_ptr + i)->activity;
            }
        }
        return activity_total;
    }

    bool alarm_control(const pillar_men_sensor* sensor_ptr){
